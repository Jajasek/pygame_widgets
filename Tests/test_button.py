...
def PretahnutiSS(event, self):
    self.attributes.drag = event.type == BUTTON_PRESSED
...
Tlaco2 = pygame_widgets.Button(Okno, size=(100, 20), text='Pretahni me', background=THECOLORS['gray30'])
Tlaco2.attributes.drag = False
Tlaco2.add_handler(BUTTON_PRESSED, button_wrapper(PretahnutiSS, event_arg=True))
Tlaco2.add_handler(BUTTON_RELEASED, button_wrapper(PretahnutiSS, event_arg=True))
Tlaco2.add_handler(MOUSEMOTION, Pretahnuti)
Tlaco = pygame_widgets.Button(Okno, (200, 150), (100, 20), text='Zmackni me')
Tlaco.add_handler(BUTTON_RELEASED, button_wrapper(Zmacknuti, (BUTTON_LEFT, BUTTON_RIGHT), True), [Okno], self_arg=False)
